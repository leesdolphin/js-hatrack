(function ($) {
  ...
  Labhr.prototype.getUserExtendedInfo = function (userInfo, contributorCallback) {
    var promise = this.cachedRequest(userInfo.url).then(function (json, status, xhr) {
      return json
    })
    promise.then(function (json) {
      contributorCallback && contributorCallback([json])
    })
    return promise
  }

  Labhr.prototype.getAllUserExtendedInfo = function (listOfUsers, contributorCallback) {
    var that = this
    var allPromises = []
    listOfUsers.forEach(function (user) {
      allPromises.push(that.getUserExtendedInfo(user, contributorCallback))
    })
  ...
  Labhr.prototype.codeContributionLoader = function (repo, contributorCallback) {
    ...
    return this.autoPaginate(baseUrl, function (listOfContributors) {
      return that.getAllUserExtendedInfo(listOfContributors, contributorCallback)
    })
  }
  ...
  Labhr.prototype.issueContributionLoader = function (repo, contributorCallback) {
    ...
    return this.autoPaginate(baseUrl, function (listOfIssues) {
      ...
      var creatorsPromise = that.getAllUserExtendedInfo(creators, contributorCallback)
  ...
  Labhr.prototype._issueCommenterLoader = function (commentsUrl, contributorCallback) {
    ...
    return this.autoPaginate(commentsUrl, function (commentsPage) {
      ...
      return that.getAllUserExtendedInfo(commenters, contributorCallback)
    })
  }
